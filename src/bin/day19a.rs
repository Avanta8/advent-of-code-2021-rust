...
/// If there are 12 intersection points, then return all the points of `b` in a matching
/// orientation and position to `a`.
fn is_intersect(a: &[Point], b: &[Point]) -> Option<Vec<Point>> {
...
fn main() {
    ...
    let mut all_points = FxHashSet::default();
    ...
    while let Some(current) = bag.pop() {
        for idx in 0..length {
            ...
            if let Some(np) = is_intersect(&current, &scanners[idx]) {
                all_points.extend(np.iter().copied());
                seen[idx] = true;
                bag.push(np);
            }
        }
